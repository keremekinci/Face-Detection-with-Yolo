...
# draw all results
def draw_boxes(filename, v_boxes, v_labels, v_scores, output_dir,faces_dir):
    file_namee = filename.rsplit("/")[1].rsplit(".")[0]
    img = cv2.imread(filename)
    for i in range(len(v_boxes)):
        box = v_boxes[i]
        y1, x1, y2, x2 = box.ymin-60, box.xmin-60, box.ymax+60, box.xmax+60
        start_point = (x1, y1)
        end_point = (x2, y2)
        ...
        face_output_dir = os.path.join(faces_dir, f"{file_namee} Faces {len(v_boxes)}")
        os.makedirs(face_output_dir, exist_ok=True)  # Create directory if it doesn't exist
        face_output_path = os.path.join(face_output_dir, f"face_{i}.jpg")
...
def create_directory(output_name,faces_dir):
    ...
    if not os.path.exists(faces_dir):
        print('==> Creating the {} directory...'.format(faces_dir))
        os.makedirs(faces_dir)
    else:
        print('==> Skipping create the {} directory...'.format(faces_dir))
...
def _main():
    ...
    input_dir = "inputs/"
    croppedFaces_dir= "faces yolo/"
    create_directory(output_dir,croppedFaces_dir)

    files = list_files_in_directory(input_dir)


    # photo_filename = "inputs\ss-1.png"
    ...
    for photo_filename in files:
        ...
        draw_boxes(photo_filename, v_boxes, v_labels, v_scores, output_dir,croppedFaces_dir)

        no_of_faces=len(v_boxes)
        print(no_of_faces)
